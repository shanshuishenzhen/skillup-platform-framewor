...
const DepartmentEditModal: React.FC<DepartmentEditModalProps> = ({
  ...
  const validateForm = () => {
    ...
    // 检查是否选择了自己或子部门作为父部门
    if (formData.parent_id && department) {
      if (formData.parent_id === department.id) {
        newErrors.parent_id = '不能选择自己作为上级部门';
      } else {
        // 检查是否选择了子部门作为父部门
        const isChildDepartment = (deptId: string, targetId: string): boolean => {
          const findInChildren = (depts: Department[]): boolean => {
            for (const dept of depts) {
              if (dept.id === targetId) return true;
              if (dept.children && findInChildren(dept.children)) return true;
            }
            return false;
          };
          
          if (department.children) {
            return findInChildren(department.children);
          }
          return false;
        };
        
        if (isChildDepartment(department.id, formData.parent_id)) {
          newErrors.parent_id = '不能选择子部门作为上级部门';
        }
      }
    }
  ...
  // 渲染部门选择选项（排除当前部门及其子部门）
  const renderDepartmentOptions = (depts: Department[], level: number = 0): JSX.Element[] => {
    const options: JSX.Element[] = [];
    
    const isExcluded = (dept: Department): boolean => {
      if (!department) return false;
      if (dept.id === department.id) return true;
      
      // 检查是否是当前部门的子部门
      const isChild = (parentDept: Department, targetId: string): boolean => {
        if (parentDept.children) {
          for (const child of parentDept.children) {
      ...
      return isChild(department, dept.id);
    };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">编辑部门</h2>
